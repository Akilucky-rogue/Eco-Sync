...
const Layout = ({ children }: LayoutProps) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-green-50/30">
        ...
        <div className="container mx-auto flex items-center justify-between py-6 px-8">
          ...
          <div className="flex items-center gap-12">
            <nav className="flex items-center space-x-8">
              {/* Main Navigation */}
              <div className="flex space-x-3 bg-white/10 rounded-lg p-2">
                {[
                  { href: "/", label: "Home" },
                  ...
                  <a 
                    key={item.href}
                    href={item.href} 
                    className="relative px-6 py-3 rounded-lg hover:bg-white/15 transition-all duration-300 group text-sm font-medium"
                  >
              ...
              <div className="flex space-x-3">
                {[
                  ...
                  <a 
                    key={item.href}
                    href={item.href} 
                    className="relative px-5 py-3 rounded-lg hover:bg-white/10 transition-all duration-300 group text-sm"
                  >
                    <span className="relative z-10">{item.label}</span>
                    <div className="absolute inset-0 bg-white/5 rounded-lg scale-0 group-hover:scale-100 transition-transform duration-300"></div>
                  </a>
              ...
              <div className="flex space-x-3 border-l border-white/20 pl-8">
                {[
                  ...
                  <a 
                    key={item.href}
                    href={item.href} 
                    className="relative px-5 py-3 rounded-lg hover:bg-white/10 transition-all duration-300 group text-sm opacity-90 hover:opacity-100"
                  >
                    <span className="relative z-10">{item.label}</span>
                    <div className="absolute inset-0 bg-white/5 rounded-lg scale-0 group-hover:scale-100 transition-transform duration-300"></div>
                  </a>
      ...
      <main className="pb-32 md:pb-8 pt-6">
        {children}
      </main>
